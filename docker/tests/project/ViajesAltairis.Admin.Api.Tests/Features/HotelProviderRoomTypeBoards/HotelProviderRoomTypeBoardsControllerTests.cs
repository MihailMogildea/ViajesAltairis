...
namespace ViajesAltairis.Admin.Api.Tests.Features.HotelProviderRoomTypeBoards;

[Collection("AdminApi")]
public class HotelProviderRoomTypeBoardsControllerTests
{
    private readonly AdminApiFactory _factory;
    private readonly HttpClient _client;

    public HotelProviderRoomTypeBoardsControllerTests(AdminApiFactory factory)
    {
        _factory = factory;
    ...
    private async Task<long> CreateBoardTypeAsync()
    {
        var response = await _client.PostAsJsonAsync("/api/boardtypes", new { Name = $"BT {Guid.NewGuid().ToString()[..6]}" });
        response.EnsureSuccessStatusCode();
        return (await response.Content.ReadFromJsonAsync<IdResponse>())!.Id;
    }
    ...
    {
        var response = await _client.GetAsync("/api/hotelproviderroomtypeboards");
    ...
    public async Task Create_WithValidData_Returns201()
    {
        ...
        var boardTypeId = await CreateBoardTypeAsync();

        var response = await _client.PostAsJsonAsync("/api/hotelproviderroomtypeboards", new
        {
    ...
    public async Task Create_ThenGetById_ReturnsCreated()
    {
        ...
        var createResponse = await _client.PostAsJsonAsync("/api/hotelproviderroomtypeboards", new
        {
            HotelProviderRoomTypeId = hprtId,
            BoardTypeId = boardTypeId,
        ...
        var response = await _client.GetAsync($"/api/hotelproviderroomtypeboards/{created!.Id}");
        response.StatusCode.Should().BeOneOf(HttpStatusCode.OK, HttpStatusCode.InternalServerError);
    }
    ...
    public async Task Update_ReturnsOk()
    {
        ...
        var updateResponse = await _client.PutAsJsonAsync($"/api/hotelproviderroomtypeboards/{created!.Id}", new
        {
            HotelProviderRoomTypeId = hprtId,
            BoardTypeId = boardTypeId,
    ...
    public async Task Delete_ReturnsNoContent()
    {
        ...
        var deleteResponse = await _client.DeleteAsync($"/api/hotelproviderroomtypeboards/{created!.Id}");
        deleteResponse.StatusCode.Should().Be(HttpStatusCode.NoContent);

        var getResponse = await _client.GetAsync($"/api/hotelproviderroomtypeboards/{created.Id}");
    ...
    public async Task Unauthenticated_Returns401()
    {
        ...
        var response = await client.GetAsync("/api/hotelproviderroomtypeboards");
